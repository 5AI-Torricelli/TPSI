...
int UDP_init(uint16_t port_number){
  ...
  if ((socket_id = socket( AF_INET, SOCK_DGRAM | SOCK_NONBLOCK, IPPROTO_UDP)) < 0) return -1;
...
int UDP_send(uint32_t ip_address, uint16_t port_number, uint8_t data[], int byte){
  ...
  if ((n = sendto (socket_id, (void *)data, byte, 0, (struct sockaddr*)&add, sizeof(add))) < 0) return -1;
  return n;
}
...
int UDP_receive(uint32_t *ip_address, uint16_t *port_number, uint8_t data[], int size){
  ...
  if ((n = recvfrom (socket_id, (void *)data, size, 0, (struct sockaddr*)&add, &dim)) <= 0) return -1;

  // estrazione indirizzo IP e numero di porta UDP
  *ip_address = (uint32_t)(ntohl(add.sin_addr.s_addr));
  *port_number = (uint16_t)(ntohs(add.sin_port));
  return n;
}
